...
export const PopupApp: React.FC = () => {
  ...
  useEffect(() => {
    // Read directly from chrome.storage so we see exactly what's stored
    chrome.storage.local.get("appSettings").then(r => {
      const s = (r["appSettings"] as {
        apiKey?: string;
        providerId?: string;
        apiModel?: string;
      }) ?? {};
      const key  = s.apiKey ?? "";
      const pid  = s.providerId ?? "anthropic";
      setApiKey(key);
      setProviderId(pid);
      const names: Record<string, string> = {
        anthropic: "Claude", openai: "OpenAI", deepseek: "DeepSeek",
        gemini: "Gemini", qwen: "千问", moonshot: "Kimi",
        zhipu: "GLM", ollama: "Ollama",
      };
      setProviderName(names[pid] ?? pid);
      setLoaded(true);
    });
  }, []);
  ...
  return (
    <div style={{ padding: "16px 14px", display: "flex", flexDirection: "column", gap: 10, minWidth: 240, fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 2 }}>
        ...
        {/* Only show badge after loaded, to avoid flicker */}
        {loaded && !hasApiKey && (
          <div style={{
            marginLeft: "auto", fontSize: 10, padding: "2px 8px",
            borderRadius: 8, backgroundColor: "#2c1f00",
            color: "#f9e2af", border: "1px solid #7c5c00", whiteSpace: "nowrap",
          }}>Demo 模式</div>
        )}
      </div>
...
const PrimaryBtn: React.FC<{ children: React.ReactNode; onClick: () => void }> = ({ children, onClick }) => (
  <button onClick={onClick} style={{
    ...
    backgroundColor: "#4f9cf9", color: "#fff", cursor: "pointer",
...
const DisabledBtn: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <button disabled style={{
    width: "100%", padding: "9px 12px", borderRadius: 8,
    border: "1px solid #313244", backgroundColor: "transparent",
    color: "#45475a", cursor: "not-allowed", fontSize: 13, textAlign: "left", fontFamily: "inherit",
  }}>{children}</button>
);
...
const SmallBtn: React.FC<{ children: React.ReactNode; onClick: () => void }> = ({ children, onClick }) => (
  <button onClick={onClick} style={{
    ...
    border: "1px solid #313244", backgroundColor: "transparent",
    color: "#a6adc8", cursor: "pointer", fontSize: 12, fontFamily: "inherit",
  }}>{children}</button>
);
