...
function extractSemanticCueText(node: Element): string {
  ...
  const parts = Array.from(node.querySelectorAll(SVG_TEXT_SELECTOR))
    .map((textNode) => normalizeMathDisplayText(
      (textNode.getAttribute("txt") || textNode.textContent || "").replace(/\s+/g, " ").trim(),
    ))
    .filter(Boolean);
...
function looksLikeQuestionTextContinuation(text: string): boolean {
  ...
  if (/(取得样本值|则参数|矩估计值|似然函数|其中是|其中为|已知|求|若|当|则|服从|样本|参数)/.test(normalized)) return true;
  return false;
}
...
function composeMathSideText(
  ...
  if (side === "denominator") {
    text = text.replace(/([A-Za-zθωσμλφψπτχXYZxyz])\s*(√\S+)\s*\//gu, "$1 / $2");
  }
  return text;
}
