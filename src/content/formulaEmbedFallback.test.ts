...
describe("formulaEmbedFallback", () => {
  ...
  it("keeps large question figures but still allows semantic continuation text after the figure", () => {
    ...
    expect(extractSemanticSvgLikeText(svg)).toContain("已知随机变量");
    expect(extractSemanticSvgLikeText(svg)).toContain("求参数");
  });
  ...
  it("reconstructs fraction plus exponent options like 5/18 theta^2", () => {
    const svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
    const makeText = (txt: string, x: string, y: string) => {
      const node = document.createElementNS("http://www.w3.org/2000/svg", "text");
      node.setAttribute("x", x);
  ...
  it("keeps theta as the baseline host for 1/4 theta^2 style options", () => {
    const svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
    const makeText = (txt: string, x: string, y: string) => {
      const node = document.createElementNS("http://www.w3.org/2000/svg", "text");
      node.setAttribute("x", x);
