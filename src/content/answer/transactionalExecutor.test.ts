...
import { attachRuntimeRoot, rootAttachmentOf, TOP_ROOT_GENERATION, TOP_ROOT_KEY } from "../roots/rootContext";
...
function resolveAuthority(block: QuestionBlock, owner: Element) {
  return () => {
    ...
    const root = rootAttachmentOf(block);
    return { ok: true as const, mapping, ...identity(block), stableId: identity(block).questionId, rootKey: root.rootKey, rootGeneration: root.rootGeneration };
  };
}
function execute(planValue: AnswerPlan, action: ActionPlan, mapping: Extract<ReturnType<typeof buildControlMapping>, { ok: true }>, block: QuestionBlock, owner: Element) {
  return executeTransaction(planValue, action, mapping, undefined, resolveAuthority(block, owner));
}
...
describe("Phase 5 stale control validation", () => {
  it("REV-C1 rejects a connected option moved into Q13 without clicking", async () => {
    ...
    expect((await execute(plan("C", block), buildActionPlan(plan("C", block), mapping), mapping, block, owner)).outcome).toBe("CONTROL_MAPPING_CHANGED");
  ...
  it("REV-C2 rejects a disabled mapped option with zero mutation", async () => {
    ...
    expect((await execute(plan("B", block), buildActionPlan(plan("B", block), mapping), mapping, block, owner)).outcome).toBe("CONTROL_MAPPING_CHANGED");
    expect(clicks).toBe(0);
  });
  ...
  it("TX-RADIO1 restores the originally selected native radio after verification failure", async () => {
    ...
    expect((await execute(plan("C", block), action, mapping, block, owner)).outcome).toBe("FILL_VERIFICATION_FAILED");
  ...
  it("TX-RADIO2 reports rollback failure when the framework blocks restoring B", async () => {
    ...
    const action: ActionPlan = { schemaVersion: 1, ...identity(block), answerSemanticHash: "C", steps: [{ type: "select-option", controlId: mapping.options.get("A")!.controlId, optionKey: "A", desiredSelected: true }] };
    expect((await execute(plan("C", block), action, mapping, block, owner)).outcome).toBe("PARTIAL_MUTATION_UNPROVABLE");
  });
});
