...
export type ControlMappingFailure = "CONTROL_MAPPING_AMBIGUOUS" | "CONTROL_MAPPING_CHANGED" | "CONTROL_NOT_FOUND" | "CONTROL_COUNT_MISMATCH";
export type ControlMappingResult = { ok: true; questionId: string; owner: Element; rootKey: string; rootGeneration: number; lifecycleToken: number; options: Map<string, ControlRef>; blanks: ControlRef[]; text: ControlRef | null; confidence: number } | { ok: false; code: ControlMappingFailure; message: string };
...
export function buildControlMapping(block: QuestionBlock, owner: Element): ControlMappingResult {
  ...
  const controlRootKey = attachedRoot?.rootKey ?? sharedRootRegistry().rootKeyOfRoot(getTraversalRoot(semanticOwner)) ?? TOP_ROOT_KEY;
  const controlRootGeneration = attachedRoot?.rootGeneration
    ?? sharedRootRegistry().get(controlRootKey)?.rootGeneration
    ?? TOP_ROOT_GENERATION;
  const lifecycleToken = controlRegistry.lifecycleToken(questionId, controlRootKey, controlRootGeneration);
  if (lifecycleToken === null) return { ok: false, code: "CONTROL_MAPPING_CHANGED", message: "Control registry scope capacity is exhausted" };
  ...
  return { ok: true, questionId, owner: semanticOwner, rootKey: controlRootKey, rootGeneration: controlRootGeneration, lifecycleToken, options, blanks, text, confidence };
}
