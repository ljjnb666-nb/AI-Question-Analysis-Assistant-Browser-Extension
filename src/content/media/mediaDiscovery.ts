...
export function collectMediaAssets(owner: Element, store: MediaPayloadStore = runtimeMediaPayloadStore): MediaAssetRef[] {
  const assets: MediaAssetRef[] = [];
  const nodes = Array.from(owner.querySelectorAll("img,canvas,svg,picture,figure,*[style*='background']"));
  for (const node of nodes) {
    if (node.tagName.toLowerCase() === "picture" || node.tagName.toLowerCase() === "figure") continue;
    const asset = discoverMediaAsset(node, owner, assets.length, store);
    if (asset && asset.ownership.role !== "decoration") assets.push(asset);
  }
  return assets;
}

export function discoverMediaAsset(element: Element, owner: Element, semanticOrder: number, store: MediaPayloadStore): MediaAssetRef | null {
  ...
  if (tag === "img") return imageAsset(element as HTMLImageElement, owner, semanticOrder, store, ownership);
  ...
  return backgroundAsset(element as HTMLElement, semanticOrder, store, ownership);
}

function imageAsset(img: HTMLImageElement, _owner: Element, order: number, store: MediaPayloadStore, ownership: MediaAssetRef["ownership"]): MediaAssetRef | null {
  ...
  return refForUrl(source, "image", effectiveKind, img, order, store, ownership);
}

function backgroundAsset(element: HTMLElement, order: number, store: MediaPayloadStore, ownership: MediaAssetRef["ownership"]): MediaAssetRef | null {
  const value = getComputedStyle(element).backgroundImage || "";
  const match = value.match(/^\s*url\(\s*["']?(.+?)["']?\s*\)\s*$/i);
  if (!match || /gradient\(/i.test(value)) return null;
  return refForUrl(match[1], "background-image", "css-background", element, order, store, ownership);
}
...
function refForUrl(source: string, kind: MediaAssetKind, sourceKind: MediaSourceKind, element: Element, order: number, store: MediaPayloadStore, ownership: MediaAssetRef["ownership"]): MediaAssetRef | null {
  ...
  if (/^blob:/i.test(source)) {
    return makeRef({ kind, sourceKind: "blob-url", availability: "unresolved", fingerprint: `blob_${stableHash(source)}`, element, order, ownership, payload: { sourceUrl: source }, store });
  }
  const canonical = canonicalizeQuestionImageUrl(source);
  if (!canonical) return null;
  return makeRef({ kind, sourceKind, availability: "url-only", fingerprint: `url_${stableHash(canonical)}`, element, order, ownership, payload: { sourceUrl: source }, store });
}

function makeRef(input: { kind: MediaAssetKind; sourceKind: MediaSourceKind; availability: MediaAvailability; mimeType?: string; fingerprint: string; element: Element; order: number; ownership: MediaAssetRef["ownership"]; payload?: { sourceUrl?: string; dataUrl?: string; serializedSvg?: string; mimeType?: string }; store: MediaPayloadStore }): MediaAssetRef {
...
export function projectLegacyMedia(block: QuestionBlock, mediaAssets: MediaAssetRef[], owner: Element): QuestionBlock {
  ...
  const displaySegments = block.displaySegments?.map((segment) => {
    ...
    return asset
      ? { ...segment, url: /^(?:data:|blob:)/i.test(segment.url) ? `media://${asset.assetId}` : segment.url, mediaAssetId: asset.assetId }
      : segment;
  });
  return { ...block, displaySegments, mediaAssets, primaryMediaAssetId: primary?.assetId, hasImage: mediaAssets.length > 0, questionImageUrl: primary ? sourceById.get(primary.assetId) : block.questionImageUrl };
}
